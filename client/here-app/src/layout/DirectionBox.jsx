...
function DirectionBox(props) {
    ...
    const onResult = function (result) {
        ...
        if (result.routes.length) {
            ...
            const startMarker = new H.map.Marker({ lat: search.origin.split(",")[0], lng: search.origin.split(",")[1] }, { icon: new H.map.Icon("https://cdn-icons-png.flaticon.com/512/7945/7945007.png", { size: { w: 56, h: 56 } }) });

            // Create a marker for the end point:
            const endMarker = new H.map.Marker({ lat: search.destination.split(",")[0], lng: search.destination.split(",")[1] }, { icon: new H.map.Icon("https://cdn-icons-png.flaticon.com/512/8830/8830930.png", { size: { w: 56, h: 56 } }) });
    ...
    return (
        <div class="w-80 h-auto rounded-b-md bg-slate-500 rounded-tr-md" >
            <div class=" p-4">
                ...
                <div class="flex gap-4 flex-col ">
                    <SearchInput
                        ...
                        setValue={(query) => setSearch((prevSearch) => ({ ...prevSearch, originName: query }))}
                        IconComponent={LocationOn}
                        handleOnChoose={handleOnClickOrigin}
                    />
                    <div class="flex flex-row justify-center">
                        <div class="hover:cursor-pointer w-8 h-8 flex justify-center items-center hover:bg-black rounded-lg" onClick={handleAltSync}>
                            <SyncAltIcon />
                        </div>
                    </div>
                    <SearchInput
                        value={search.destinationName}
                        setValue={(query) => setSearch((prevSearch) => ({ ...prevSearch, destinationName: query }))}
                        IconComponent={LocationOn}
                        handleOnChoose={handleOnClickDestination}
                    />
