    ...
    <div className="mt-2">
      ...
      <div className='flex flex-wrap mt-2 bg-bg gap-3'>
        {reviews?.slice(0, 5)?.map((review, index) => (
          <div key={index} className="p-4  max-w-full border-2 border-scaffold rounded-2xl">
            <div className="flex flex-row">
              <img className='rounded-full w-20 h-20 object-cover border-4 border-primary' src={review.author.picture} alt="" />
                ...
                <div className="flex-auto justify-end">
                  <div className='flex flex-row items-center justify-end'>
                    {Array.from({ length: 5 }, (_, index) => index + 1).map(index => (
                      <div key={index} className={(index <= review.rating / 2) ? 'text-yellow' : 'text-text'}>
                        <Star />
                      </div>
                    ))}
                  </div>
